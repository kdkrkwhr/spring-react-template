...
public class UserRequestDto {
  ...
  public User toEntity() {
    ...
    return User.builder().authorities(this.getAuthorities()).identifier(this.getIdentifier())
        .password(this.getPassword()).name(this.getName()).email(this.getEmail()).savings(0)
        .roadAddr(this.getRoadAddr()).buildingName(this.getBuildingName())
        .detailAddr(this.getDetailAddr()).disabledYn('N').build();
  }
}
